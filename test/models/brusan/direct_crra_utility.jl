...
using Test, OrdinaryDiffEq, HDF5, ModelConstructors, Plots, ForwardDiff, NLsolve, Statistics, FiniteDiff
include(joinpath(dirname(@__FILE__), "../../../src/includeall.jl"))
=#
start_time = time_ns()

default = false # fastest
forwarddiff = false # slowest
finitediff = true # close to default
nlsolve_newton = false
nlsolve_trustreg = false
nlsolve_newton_qonly = false
nlsolve_trustreg_qonly = false
...
forwarddiff_err = fill(NaN, N, 3)
finitediff_err = fill(NaN, N, 3)
nlsolve_newton_err = fill(NaN, N, 3)
nlsolve_trustreg_err = fill(NaN, N, 3)
nlsolve_newton_qonly_err = fill(NaN, N, 3)
nlsolve_trustreg_qonly_err = fill(NaN, N, 3)
...
for t in 1:T
    ...
    for n = 2:N
        ...
        # errors given guess
        if forwarddiff || finitediff
            staticstep!(F, xₙ₋₁, η[n], dη[n - 1], q_old, GS[n], VVlp[n - 1], 1)
            if forwarddiff
                QN = ForwardDiff.jacobian((F, x) -> staticstep!(F, x, η[n], dη[n - 1], q_old, GS[n], VVlp[n - 1], 1), F, xₙ₋₁)
            elseif finitediff
                QN = Matrix{eltype(xₙ₋₁)}(undef, length(xₙ₋₁), length(xₙ₋₁))
                FiniteDiff.finite_difference_jacobian!(QN, (F, x) -> staticstep!(F, x, η[n], dη[n - 1], q_old, GS[n], VVlp[n - 1], 1), xₙ₋₁)
            end
